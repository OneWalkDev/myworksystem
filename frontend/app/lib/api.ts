...
export const api = {
  ...
  async getCasePriority(token: string) {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch case priorities: ${response.statusText}`);
    }
    return response.json();
  },
  ...
  async getCases(token: string, params?: {
    page?: number;
    per_page?: number;
    name?: string;
    client_name?: string;
    ...
    const url = `${API_BASE_URL}/cases${queryParams.toString() ? `?${queryParams.toString()}` : ""}`;
